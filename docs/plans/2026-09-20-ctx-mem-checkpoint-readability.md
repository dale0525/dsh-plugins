...
> 该流程尚未执行，端到端验收（A14）**未通过**（§10.6）；走完该流程后复验**已通过**（§10.7）。
> 复验同时暴露两处**新发现的缺陷**（S6 配对上下文不限行数、错误条目可能取到进度行），
> 二者均已修并复验（§10.8；配对上下文的取行口径另见 §10.8 的回归说明与 A22）。
...
| **A22** | S6 的上下文在首行不构成自足句子时**保留首行 + 末行**；保留文本受 `contextCap` 约束（只额外加标记），且仍是单一物理行 | 单元测试：引导语 + 多行正文的 `↑` 行同时含首末行、`…` 标记存在、保留文本长度 ≤ `contextCap`；路径/版本号里的 `.` 不算句末；超长首行仍标记未容纳的末行；两行都完整时不报丢弃；标题形末行不落在行首 | **已满足**（`render.test.js` 七条 A22 用例） |
...
| **A16** | 设置界面能改该键并持久化，重启后生效 | 真实界面验证（A14） | **实现已落地**（`src/client.js` + `installSection`）；界面**未经真实点击验证**——见 §10.7 |
...
> **2026-09-21 实测：首次核对未通过**——根因是改动未发布，见 §10.6。**发布 0.3.0 / 0.5.4
> 并 `dsh-web restart` 后复验通过**：重启后产出的两折全部满足四项契约，重启前的四折仍是旧形态。
> 复验证据、对照表与新发现的两处缺陷见 §10.7。
...
| **落地≠生效：改动未发布时 GUI 仍跑旧版** | A14 端到端验证失败——压缩产物仍是旧形态 | 实测见 §10.6：本会话（2026-09-21）真实压缩产出的检查点**仍是 errors 在 commands 之后、无 `↑` 配对行**。根因是 profile 装的是**已发布的 0.2.0**（== HEAD，pre-S5/S6），而工作树改动**未提交、未升版、未发布**。生效路径：提交 → 升版 → CI/CD 发布 → `dsh-web restart`。**该风险已于同日走完生效路径并复验通过（§10.7）** |
...
S5 (config.js 缺省值 + 客户端半边 + installSection) ── 已落地
                                        │
                                        ▼
提交 → 升版 → CI/CD 发布 → dsh-web restart        ── 已完成（A14 复验通过，§10.7）
```
...
5. ~~把工作树改动提交、升版、走 CI/CD 发布，然后重启宿主。~~ 已完成（commit `8d4e282`、
   tag `v0.5.4`、npm `0.3.0`/`0.5.4`、`dsh-web restart`），A14 复验通过（§10.7）。
...
> **后续**：上表中的提交、升版、发布三步已于同日执行完毕，复验结果见 §10.7。

### 10.7 发布后复验（A14）——**已通过，并发现两处新缺陷**（2026-09-21）

**触发**：用户要求走完「提交、升版本、发布、本地安装、重启、本地验收」。§10.6 列出的三步全部执行：

| 步骤 | 结果 |
|---|---|
| 提交 | `8d4e282`（S0–S6 全部改动），已推 `origin/main` |
| 升版 | `@logictan/dsh-ctx-mem` 0.2.0 → **0.3.0**（新增 `./client` 导出，走 minor）；聚合包 0.5.3 → **0.5.4**，dependencies 收敛到 `^0.3.0` |
| 发布 | CI/CD（tag `v0.5.4`）；npm 上 `0.3.0` 与 `0.5.4` 均可解析 |
| 本地安装 | profile `web` 装 `@logictan/dsh-plugins-all@0.5.4`，`dsh-ctx-mem@0.3.0` 落盘，`lib/client.js` 存在 |
| 重启 | `dsh-web restart`；`dsh-web status` = `Verdict: OK`（job PID == 端口 owner 30309） |

**核对方法**：解压会话归档 `session-b21ec2c4…/session.v3.jsonl.zstd`（本会话自身，含重启前后各折），
逐折读出 `compaction/summary` 的投递正文，按 §6 的四项契约判定。

**结果：重启后的两折全部通过，重启前的四折全部不通过**——边界干净地落在 `dsh-web restart` 上：

| 折 seq | 时刻 | errors 在 commands 之前 | 命令节非写类条目 | 配对 `↑` 行 | 判定 |
|---|---|---|---|---|---|
| 510 / 1010 / 1348 / 1532 | 00:49–01:30（重启前） | ✘ | 59 / 100 / 100 / 100 | 0 | 旧形态 |
| 2347 | 02:41 | ✔ | 0 / 31 | 2 | **通过** |
| 2794 | 02:54 | ✔ | 0 / 31 | 3 | **通过** |

命令节的「非写类条目」按 `isWriteLike` 逐条复算：重启前四折分别有 59–100 条不满足
（`git status`、`wc -l`、`grep -n`、`sed -n` 一类只读探针），重启后两折为 **0**——
S0 的探针丢弃在真实产物上生效。`### Errors Seen` 的偏移也从 18,356/19,093 前移到 2,436/2,685，
即 S4 的节序调整生效。

**复验暴露的两处缺陷（当时均未修，后于 §10.8 修复）**：

1. **S6 的配对上下文没有限行数。** `contextCap`（T1 = 300 字符）是**字符**上限，不是行数上限：
   一条 300 字符的 assistant 陈述可以横跨十几行，其首行若是 `## 结论`，在产物里就读成新标题
   （实测 seq 2794 的 intents 第 3 条）。§5.5 与 A20 只钉住「intent 不裁剪、context 受 cap」，
   **没有钉住「配对上下文是单行」**——这是判据缺口，不是实现违约。
2. **错误条目的取行口径可能取到进度行。** `firstMeaningfulLine` 取结果的首个非空行；
   pnpm 把 `✓ Lockfile passes supply-chain policies` 打印在真正的报错之前，于是该条目渲染为
   `bash: ✓ Lockfile passes…`。该条目**本身是真错误**（同一条结果后段是
...
> **本修法的回归（2026-09-21，A22）。** 「只取首个非空行」把 §8.2 已裁定的「保留整条并截断」
> 收得过紧：assistant 陈述若以**引导语**开头（`修复完成。`、`## 推荐方案：…`），首行本身不承载
> 内容，而被问到的结论在末行 —— 用户报的实例即 `修复完成。` 之后第 4 段才是
> `## 顺带发现（未改，仅报告）`。现改为：首行**不超过 25 字符**，或去掉 Markdown 强调/引号后
> **不以句末标点结尾**（即不构成自足句子）时，保留首行 + 末行，中间以 ` … ` 标记省略。
> 保留文本仍受 `contextCap` 约束（**只额外加标记**），且仍是单一物理行（S2 的标题形上下文防线
> 不破）。「句末」必须是**锚定**的：路径里的 `.`（`…/render.js 的取行逻辑`）与版本号里的 `.`
> （`v1.2.3，接下来…`）都不算句子结束 —— 实测 296 条多行陈述中有 **15 条**会被「含标点即自足」
> 的宽松判据误判为自足。
>
> **实测口径**（2026-09-21，693 份会话日志 / 526 条配对上下文，脚本 `extractFacts` → 分类）：
> 首行为引导语的 211 条（40.1%），其中**末行与首行不同**（即本修法真正修复）的 **131 条**
> （占全部配对 24.9%、占多行陈述 44.3%）；末行可见率 0% → 99.2%（余下 1 条首行 353 字符
> 超 `contextCap`，已按超长首行截断并标记）；content-free 从 40.1% 降到 15.8%；
> 成本 +14,056 字符 ≈ +4,016 token（每条 +7.6）。
