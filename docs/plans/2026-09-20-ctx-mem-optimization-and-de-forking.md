...
> 状态：事前冻结的计划，待用户裁定 §8 的未决问题后开工。
> 实施中的偏离以实际代码为准。全部数字来自 §4 的实测，探针与口径随文标注。
...
12 轮不触地板、最低保留率 87.4%。** 教训：模拟「分母」时必须问「宿主这一轮会重新定价哪些事件」，
而不是只定价自己的产出。
...
最低保留率 84.2%（512）～92.0%（64）。保留率随余量单调下降，故取最小的、仍非零的 64
...
| A15 忠实链 12 轮 | 不触地板、最低保留率 87.4% **PASS** |
| 单元测试 | **132/132**（S1 前为 106） |
