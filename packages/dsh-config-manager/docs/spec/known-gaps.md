...
| G-08 | 密码强度校验（基线形同虚设 → 一度接通 → 随加密层整体移除） | ➖ **加密层已整体删除**，强度校验随之不存在（本实现 不产生也不解密加密产物） |
...
> **最终状态（当前工作区，也是本条的结论）**：**本实现 已删除整个加密层**，因此**不存在密码**，也就没有「强度校验」这回事。导出与同步恒为明文。
>
...
| ~~整体加密容器 `DCA1`~~ | **已不适用**：本实现 删除了加密层（`encryptArchive` 等符号已不存在），本实现不产生也不解密加密产物 |
