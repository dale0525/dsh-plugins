...
| G-08 | 密码强度校验（基线形同虚设 → 一度接通 → 随加密层整体移除） | ➖ **加密层已整体删除**，强度校验随之不存在（本 fork 不产生也不解密加密产物） |
...
>
> **G-08 是本节的特例**：它不是「被修好」，而是**该能力连同整个加密层被删除**。它保留在本节只为记录完整历史，**不要**读作「闸门已接通」。
...
> **最终状态（当前工作区，也是本条的结论）**：**本 fork 已删除整个加密层**，因此**不存在密码**，也就没有「强度校验」这回事。导出与同步恒为明文。
>
> **这不是缺口，也不再是「修复」**——这是产品负责人（用户）的明确决策：同步通道是私有通道，明文自用比丢配置更划算。G-08 因此**从缺口登记中退出**，保留此条只为记录历史。
...
| **最终态（产品决策，2026-09-13）** | **上述闸门被整体移除**，而非保留或收紧。用户原话：「我需要加密备份时无需校验，无论是什么密码都可以使用。」 |
| 移除物（全库零残留） | 强度校验函数与断言函数、**整个 `src/security/encryption.ts`**（`EncryptionProvider` / `createEncryptionProvider` / `encryptArchive` / `isArchiveBlob` 等）、导出路由的密码守卫、400 弱密码拒绝块、`config_backup` 的弱密码结构化拒绝、消息键 `export.passwordTooWeak` / `export.encryptionRequired`、以及对应的测试用例。**以上符号在当前工作区均已不存在，不得再被引用。** |
| 为什么移除而非接通 | 避免留下「看起来在守、实际不跑」的死代码。若产品不要求强度闸门，正确做法是**删掉**，而不是接通它再放宽到形同虚设。 |
| 验证方式 | 全库检索被移除符号**零命中**（`createEncryptionProvider` / `encryptArchive` / `isArchiveBlob` / `export.encryptionRequired`）；`src/security/` 下已无 `encryption.ts`；`manifest.security.encrypted` 恒 `false`、`security.encryption` 恒 `null`（`src/core/exporter.ts:330-331`）。 |
| 残留边界（**历史产物仍可被识别**） | 上游历史备份可能用弱密码加密，本实现**不提供解密**——必须先用外部工具解密成明文 ZIP。但 `security.encrypted === true` 的历史产物**不会被静默当明文导入**：`src/core/analyzer.ts:533-535` 在未注入解密结果时拒绝执行（规格 §4.7）。 |
...
| 项 | 内容 |
|---|---|
| 问题 | `sections.secrets` 永远是 `false`（无 adapter）；「是否含凭据」由 `security.containsSecrets` 承载。第三方若按「`sections.secrets === true` 表示含凭据」实现，会判断错误。 |
| 收口方式（**规格侧，实现刻意未变**） | `buildSectionFlags` 里 `flags['secrets'] = false` 仍在（当前工作区 `src/core/exporter.ts:372`）——这不是实现缺陷，而是「键集合恒全量」的**语义设计**。规格 §2.5 已加显式警告块；§9 步骤 4 的验收判据明确「判断『含秘密/需密码』只看 `security.containsSecrets` / `security.encrypted`，不看 `sections.secrets`」。 |
...
| ~~整体加密容器 `DCA1`~~ | **已不适用**：本 fork 删除了加密层（`encryptArchive` 等符号已不存在），本实现不产生也不解密加密产物 |
