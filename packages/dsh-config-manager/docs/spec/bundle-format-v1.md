...
> - 取证插件版本：`dsh-config-manager@0.1.59`（`package.json:3`）；§1 的 `src/utils/zip.ts` 行号已按 **0.1.71** 重新校准（该版本删除了 `maxRatio`）
...
| 历史密文条目 / 整体加密容器的字节布局（**本实现已不产生**，见 §4） | §4.2（识别用布局；上游实现已从本实现 删除） |
...
> - ~~密码强度校验函数在 host 侧的实际调用点~~ → **已不再适用**：该函数与整个加密层都已从本实现 源码移除（见 §4 与 §10 G-08）。现状不是「调用点在哪」，而是**没有加密这回事**——本实现不产生、不解密任何加密产物。
...
Bundle Format v1 的**文件布局、manifest 契约、checksums 语义、ZIP 读侧强制/容忍边界（§1.5）、文件类条目名归一化规则（§3.3.1）、各分区未知字段的真实命运（§7.3）、`redactedHits` 的非持久化语义（§5.3）**都已写明，第三方可以照 §1–§6 实现一个兼容的 exporter/importer。§4 只描述**上游历史加密产物**的识别方法（本实现 不产生也不解密）。
...
- **G-08「加密密码强度」已随加密层整体消失**——本实现 删除了加密层本身（§4），强度校验函数不再存在，**也没有密码这回事**（§10 G-08）。**第三方实现者不要假设本格式对加密密码有强度要求，也不要在自己的实现里加闸门**——那会与格式行为不一致，并可能让用户在跨实现迁移备份时被拒。
