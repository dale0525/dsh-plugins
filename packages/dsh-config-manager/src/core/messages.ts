...
export const zh = {
  ...
  'adapter.dirsExcluded': '{type} 分区：已按设计跳过 {count} 个缓存与会话历史目录（**有意不进备份**）—— 缓存类下次启动重建；会话历史/工作现场**不会重建**，目标机没有这些历史 —— {detail}',
  'adapter.filesExcluded': '{type} 分区：已按设计跳过 {count} 个瞬时状态文件（**有意不进备份**）—— SQLite 的 -shm/-wal 边车属于数据库瞬时状态，随进程关闭回收，不属于配置 —— {detail}',
  ...
  'sync.localSnapshotPruneFailed': '本地旧快照 {id} 清理失败（不影响本次同步）: {reason}',
  ...
  'sync.git.cleanFailed': 'git clean 失败: {err}',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'adapter.dirsExcluded': '{type}: skipped {count} cache and conversation-history director(ies) by design (intentionally NOT in this backup) — caches are rebuilt on next start; conversation history/scratch is NOT rebuilt, so the target device will not have it — {detail}',
  'adapter.filesExcluded': '{type}: skipped {count} transient-state file(s) by design (intentionally NOT in this backup) — SQLite -shm/-wal sidecars are database transient state reclaimed when the process closes, not configuration — {detail}',
  ...
  'sync.localSnapshotPruneFailed': 'Failed to remove old local snapshot {id} (does not affect this sync): {reason}',
  ...
  'sync.git.cleanFailed': 'git clean failed: {err}',
