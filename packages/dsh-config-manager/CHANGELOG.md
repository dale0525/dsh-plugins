...
`pluginFiles` 的 `collectDir`（`plugin-config`）是插件共享的跨设备配置容器，但插件会把**整个托管 HOME**
放进它（agy-link 每个账号一个）。实测本机 `~/.dsh/plugin-config` 遍历出 **3,920 MB / 102,582 文件**，
其中约 3.9 GB 是缓存与历史，导致一次推送在收集阶段就耗时数分钟到数十分钟；且 `scratch` 内嵌的 git pack
单文件 **128.5 MB** 已超 GitHub 的 100 MiB 硬上限，**该次推送注定失败**。

**新行为**：`collectDir` 收集按**路径形状**剪枝已知运行时目录（`PLUGIN_FILES_EXCLUDED_DIRS`）：
`Library/Caches`、`.npm`、`.gemini/antigravity-cli/{conversations,scratch,brain,log,presence,implicit,cache,updater,annotations,mcp}`。

| 指标 | 剪枝前 | 剪枝后 |
|---|---|---|
| 文件数 | 102,582 | **1,626** |
| 体积 | 3,920 MB | **50.9 MB** |
| 遍历耗时 | 6,279 ms | **212 ms** |
| 最大单文件 | 128.5 MB（超限） | **12.2 MB** |

**跨设备可用性**：账号身份全部保留 —— `antigravity-oauth-token`、`settings.json`、`.gemini/config/**`
（含 skills）、`Library/Keychains/login.keychain-db`、`pool.json`，所以目标机**可直接登录使用**。
`pool.json` 的绝对路径在加载时按 `basename` 重定位（`pool.ts` 的 `load()` 注释明确覆盖「同步到另一台
机器」）。

**代价必须说清楚**：被剪内容分两类，后果不同 —— 缓存类（`Library/Caches`、`.npm`、`log`、MCP 工具清单）
下次启动重建，无损失；但 agy 的**会话历史与工作现场**（`conversations` / `brain` / `annotations` / `scratch`，
合计约 1.4 GB）**不会被重建，排除即永久丢失** —— 目标机账号能用，但**没有这些历史会话**。报告文案
（`adapter.dirsExcluded`）如实分列这两类，不再断言「不影响使用」。

**排除清单对链接同样成立**：不仅看条目路径，也看链接**解析后的目标**。否则 `cli.log -> log/cli-*.log`
这类文件链接会把已剪目录的内容带进包，而报告却宣称该目录已跳过。

**只影响导出**：`applyItem` 只写快照中存在的文件，**从不删除**本地文件，本机缓存不会被清掉。

**报告语义分离**：被剪目录记入新增的 `RecursiveListing.excludedDirs`，与表示内容缺失的 `skippedLinks` /
`unreadableDirs` **分列告警**（`adapter.dirsExcluded` 不含「未进备份」措辞）—— 策略与缺失混为一谈
正是 issue #37 的教训。

### Fix: push hung for tens of minutes — the plugin config dir was syncing 3.9 GB of runtime cache

`pluginFiles`'s `collectDir` (`plugin-config`) is the shared cross-device config container, but plugins put a
**whole managed HOME** in it (one per agy-link account). On this machine it walked **3,920 MB / 102,582 files**,
~3.9 GB of which is cache and history; a single embedded git pack (**128.5 MB**) already exceeded GitHub’s
100 MiB hard limit, so **that push could never have succeeded**.

The `collectDir` walk now prunes **by path shape** (`PLUGIN_FILES_EXCLUDED_DIRS`), and the exclusion also holds
for links whose resolved target lands in a pruned directory. Account identity is fully retained (token,
`settings.json`, `.gemini/config/**`, `login.keychain-db`, `pool.json`), so the target device can log in and work
immediately.

**The cost is stated honestly**: caches are rebuilt on next start, but agy conversation history and scratch
(`conversations` / `brain` / `annotations` / `scratch`, ~1.4 GB) are **not** rebuilt — excluding them loses that
history permanently. The account works on the target device; the history does not come with it.

Pruning affects **export only** — `applyItem` never deletes local files. Pruned directories are reported through a
new `excludedDirs` field, listed **separately** from content-loss warnings.
