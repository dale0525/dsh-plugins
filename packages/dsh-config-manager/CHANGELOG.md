...
**跨设备可用性不受影响**：账号身份全部保留 —— `antigravity-oauth-token`、`settings.json`、
`.gemini/config/**`（含 skills）、`Library/Keychains/login.keychain-db`、`pool.json`。被剪目录均可由插件
在目标机重新生成。`pool.json` 的绝对路径在加载时按 `basename` 重定位（`pool.ts` 的 `load()` 注释明确
覆盖「同步到另一台机器」），`agentHome` 在启动时自愈。
...
The `collectDir` walk now prunes known runtime directories **by path shape** (`PLUGIN_FILES_EXCLUDED_DIRS`).
Account identity is fully retained (token, `settings.json`, `.gemini/config/**`, `login.keychain-db`, `pool.json`);
pruned directories are regenerated by the plugin on the target machine. `pool.json` absolute paths are re-based
on load and `agentHome` self-heals at boot, so cross-device use still works.
