...
Published as [`@logictan/dsh-openviking`](https://www.npmjs.com/package/@logictan/dsh-openviking).

## Install

This is the DSH fork of the upstream OpenViking memory plugin. It ships inside
the aggregate bundle, which is the supported way to install it:
...
```bash
dsh plugin --profile web add @logictan/dsh-openviking
```
...
packages/dsh-openviking`) only works when that checkout has its own
`node_modules`, because Node resolves the bundle's dsh peers from the source
tree's realpath rather than from the profile.
It also needs `node packages/dsh-openviking-shared/sync.mjs` run first: the
...
`node packages/dsh-openviking-shared/sync.mjs` — edit
`packages/dsh-openviking-shared/lib/`, never the vendored copies.
...
dsh plugin --profile web add @logictan/dsh-openviking@latest
dsh --profile web
```
...
durable, source-attributed user messages (`source: { kind: 'plugin:openviking-memory', … }`).
