...
// Internals re-exported for the smoke test and host-side debugging; the plugin
 ...
 * host derives an editable form for this entry and commits settings edits into
 * the running fiber instead of remounting the plugin.
 */
 ...
 *
 * `Config` is marked volatile at its root, so the loader hands the plugin one
 * reference whose value is updated in place by `loader/volatile-update`
 * instead of remounting the plugin on every settings edit.
 */
 ...
 * @param config - live config reference (schema defaults applied by the loader).
 */
export function apply(ctx: Context, config?: ConfigRef<Config>): void {
  // The live source every surface reads; the reference is updated in place by
  ...
  // keeps cancellation and retries coherent across both entry points; Agent
  // tools wait for their task result by default and render images in the tool
  // result instead of injecting a synthetic user message.
  const runtime = new ImageGenerationRuntime(channelsView)
  const pendingConversationImages = new Map<string, ImageAttachmentRef>()

  // The route family mounts once, gated on the settings seam (the bridge serves
  // it; without the seam there is nothing to expose). Route handlers read
  // resolve() per request, so config edits apply live. The settings bridge
  // deliberately keeps serving while the plugin is disabled — it is how the
