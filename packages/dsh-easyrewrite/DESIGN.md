...
- 目标消息是**会话第一条**（之前没有任何历史）时无边界可截断：不 fork，改为归档当前会话 + 打开空白新会话，修改稿即新会话的第一条（§5 同名行）。**绝不**把修改稿追加到任何既有历史末尾。
...
| 场景 | 处理 |
|---|---|
| 会话第一条用户消息 | 无更早闭合边界 → **无缝重置**：归档当前会话 → 在工作区打开空白新会话 → 修改后的内容作为新会话第一条自动发出（撤回则仅回填输入框，等发送时执行）。不做按钮置灰——用户不必先手工清空历史 |
...
- [x] 同一会话单待定约束；运行中禁用（回合未闭合报错）；首条消息无边界 → 无缝重置（归档 + 空白新会话，修改稿作为新会话第一条）；图片撤回仅文本回填
