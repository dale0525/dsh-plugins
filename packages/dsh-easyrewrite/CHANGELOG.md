...
## [2.7.1] — 修复首条消息编辑/撤回被追加到旧历史末尾

### 修复
- **【核心】编辑会话第一条消息后发送，修改稿不再被追加到旧历史末尾**：首条消息之前没有闭合回合边界，`/bubble/recall` 返回 `no-boundary` 后走重置路径。此前该路径先判定「家族父版本」并把修改稿经 resume 机制发到**父版本历史的末尾**——于是「编辑首条并发送」的结果变成「整段旧对话 + 修改稿」，与 §2.3 的截断语义（修改后的消息即最后一次发送）正好相反。现改为直接归档当前会话 + 在工作区打开空白新会话，修改稿作为新会话的**第一条**发出。
- **【核心】修复「空白新会话」通道恒不可达**：`apply` 中 `ctxUiWorkspaceRef = ctx.uiWorkspace || ctx.get("uiWorkspace")` 把未注入的 `ctx.uiWorkspace` 放在 `||` 左侧。cordis 对未 inject 的服务属性访问直接抛错（`cannot get property "uiWorkspace" without inject`），异常被 `catch` 吞掉，`ctx.get("uiWorkspace")` 永远执行不到，`ctxUiWorkspaceRef` 恒为 `null` —— 空白新会话通道自 2.6.0（`0fabd7e3` 引入该取值式）起失效，重置只能落到「归档后回 hero」的兜底路径。现改为只走 `ctx.get("uiWorkspace")`。
- **重置提示文案同步**：`resetNotice` 原文案仍描述旧的「回到上一次模型回复处」，与修复后的行为不符，三种语言一并改为「修改后的内容将作为新对话的第一条发出」。

### 说明
- DESIGN §5「会话第一条用户消息 → 编辑/撤回禁用，按钮置灰」与 §2.3「修改后的消息即最后一次发送」互相矛盾，且置灰会让用户被迫手工清空历史。裁定取 §2.3 的语义（重置而非置灰），DESIGN §2.3/§5 已同步。
- 撤回键在首条消息上现在正常进入「待发送」态（此前会直接触发重置并丢失历史）；撤回只回填输入框，真正截断发生在发送时（§2.2 惰性提交）。

## [2.7.0] — 编辑入口改为显式按钮（点气泡不再进入编辑）

### 行为变更
- **默认关闭「气泡框编辑」**：`rewriteOnClick` 默认值由 `true` 改为 `false`。单击消息气泡不再进入原位编辑，改为在操作区显示显式的「编辑」键——与主流 harness 客户端（消息下方悬停出编辑按钮）一致。
  - 旧行为（点气泡即编辑）仍可在设置页「气泡框编辑」中开启。
  - 该设置关闭时，气泡不再显示 `点击编辑` 悬浮提示（避免出现「提示可点、实际无效」的失实提示）。

### 修复
- **操作键文案本地化**：「撤回」「编辑」两个操作键的 `title` / `aria-label` 原先硬编码中文，现改用 `L.sectionRecall` / `L.sectionEdit`，英文与日文界面下同步正确显示。
  - 因「编辑」键现在默认可见，此前的硬编码问题从「次要」升级为「默认界面上可见」，故随本次行为变更一并修复。
