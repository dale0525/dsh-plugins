...
function timeoutMessage(t: UiT): string {
  return t('error.lifecycleTimeout', { minutes: String(Math.round(LIFECYCLE_TIMEOUT_MS / 60000)) })
}
...
async function getJson<T>(path: string, t: UiT): Promise<T> {
...
export class LifecycleApi {
  ...
  async status(): Promise<LifecycleStatus> {
    return getJson<LifecycleStatus>(LIFECYCLE_API.status, this.t)
  }
  ...
  async undo(): Promise<LifecycleOutcome> {
    return postJson<LifecycleOutcome>(LIFECYCLE_API.undo, {}, this.t)
  }

  /** POST /lifecycle/redo：重做上一次撤销（仅当撤销后没有新变更时可成功）。 */
  async redo(): Promise<LifecycleOutcome> {
    return postJson<LifecycleOutcome>(LIFECYCLE_API.redo, {}, this.t)
  }
  ...
  async crash(): Promise<CrashReport> {
    return getJson<CrashReport>(LIFECYCLE_API.crash, this.t)
  }

  /** GET /rescue：救援模式状态。 */
  async rescueStatus(): Promise<RescueStatus> {
    return getJson<RescueStatus>(LIFECYCLE_API.rescue, this.t)
  }
  ...
  async rescueOn(): Promise<RescueActionResult> {
    return postJson<RescueActionResult>(LIFECYCLE_API.rescue, { action: 'on', confirm: true }, this.t)
  }

  /** POST /rescue {action:'off'}：退出救援模式（从备份还原）。 */
  async rescueOff(): Promise<RescueActionResult> {
    return postJson<RescueActionResult>(LIFECYCLE_API.rescue, { action: 'off' }, this.t)
  }
}
